...
Engineering response: change the benchmark order to:
...
`MAP_TANK_FIRST_BENCH_v0_1_1_TEST` is the historical filename of the controlled benchmark used for this proof.
...
The Wi-Fi sequential phase looked stalled because it took much longer than Ethernet, but Wireshark continued showing sustained traffic and the benchmark completed normally.
...
ArcGIS Earth added the TPKX as an ordinary file through the Windows file picker using the Samba path.
...
The Android client must earn its own acceptance. Do not add a field GIS server merely because the mobile path still needs work.
